...
Sequence = Union[List[SequenceT], Tuple[SequenceT, ...]]
Page = Union[
    str,
    Sequence[str],
    discord.Embed,
...
class ButtonPaginator(Generic[PageT_co], discord.ui.View):
    ...
    def update_buttons(self) -> None:
        if self.max_pages < 2 or self.current_page <= 0:
            self.previous_page.disabled = True
            self.first_page.disabled = True
        else:
            self.previous_page.disabled = False
            self.first_page.disabled = False
        if self.max_pages < 2 or self.current_page >= self.max_pages - 1:
    ...
    @discord.ui.button(label="First", style=discord.ButtonStyle.grey, emoji="⏪")
